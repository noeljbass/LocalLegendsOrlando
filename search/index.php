...
<?php if ($query): ?><section class="section"><div class="section-heading"><div><p class="eyebrow">Search results</p><h2><?= count($results) ? count($results) . ' stories for “' . e($query) . '”' : 'No stories found' ?></h2></div></div><?php if ($results): ?><div class="story-grid story-grid-three"><?php foreach ($results as $article): ?><article class="story-card"><a class="card-image" href="<?= article_public_url($article) ?>"><img src="<?= e(media_url($article['image'] ?? null)) ?>" alt="" loading="lazy"></a><div class="card-body"><p class="kicker">Community spotlight</p><h2><a href="<?= article_public_url($article) ?>"><?= e($article['title']) ?></a></h2><p><?= e($article['excerpt'] ?: excerpt($article['content'])) ?></p><a class="text-link" href="<?= article_public_url($article) ?>">Read the story →</a></div></article><?php endforeach; ?></div><?php else: ?><p>Try a business name, a service, a category, or a tag. You can also <a href="<?= url('stories/') ?>">browse all stories</a>.</p><?php endif; ?></section><?php endif; ?>
