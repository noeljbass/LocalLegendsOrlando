...
$pageTitle='Local Legends Orlando | Stories that strengthen our city'; require 'includes/header.php'; $stories = public_stories(3); $articles = public_editorial_articles(3); $categories = public_categories() ?: homepage_categories(); ?>
...
<section class="section featured"><div class="section-heading"><div><p class="eyebrow">Local insights</p><h2>Featured articles</h2></div><a class="text-link" href="<?= url('articles/') ?>">View all articles <span>→</span></a></div><div class="story-grid"><?php foreach ($articles as $index=>$article): ?><article class="story-card <?= $index===0 ? 'story-card-large' : '' ?>"><a href="<?= article_public_url($article) ?>" class="card-image"><img src="<?= media_url($article['image'] ?? null) ?>" alt="" loading="lazy"></a><div class="card-body"><p class="kicker">Article</p><h3><a href="<?= article_public_url($article) ?>"><?= e($article['title']) ?></a></h3><p><?= e($article['excerpt']) ?></p><a class="text-link" href="<?= article_public_url($article) ?>">Read article <span>→</span></a></div></article><?php endforeach; ?></div></section>
<section class="section category-band"><div class="section-heading"><div><p class="eyebrow">Find your next favorite</p><h2>Explore by category</h2></div></div><div class="category-grid"><?php foreach ($categories as $index => $category): ?><a class="category-card cat-<?= ($index % 4) + 1 ?>" href="<?= url('category/'.$category['slug'].'/') ?>"><span><?= e($category['name']) ?></span><small><?= e($category['description'] ?: 'Discover local stories in this community.') ?></small><b><?= isset($category['article_count']) ? e((string)$category['article_count']) . ' stories' : 'Explore' ?> →</b></a><?php endforeach; ?></div></section>
