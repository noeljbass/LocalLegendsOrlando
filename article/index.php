...
$profileType = ($article['profile_type'] ?? 'company') === 'author' ? 'author' : 'company';
$publisherName = trim((string) ($article['profile_display_name'] ?? ''));
if ($publisherName === '') $publisherName = preg_replace('/:\s*A Local Legends Story$/i', '', (string) ($article['title'] ?? '')) ?: ($categories ? $categories[0]['name'] : 'Local Legends Orlando');
$profileLabel = trim((string) ($article['profile_label'] ?? ''));
$profileBio = trim((string) ($article['profile_bio'] ?? ''));
$profileAriaLabel = $profileType === 'author' ? 'Author profile' : 'Featured company profile';
$profileWebsiteLabel = $profileType === 'author' ? 'Visit author website' : 'Visit website';
?>
